...
def load_financieras() -> pd.DataFrame:
    ...
    hoja = "DATASET_PRELIMINAR_134"
    if hoja not in xl.sheet_names:
        if len(xl.sheet_names) == 1:
            log(f"FINANCIERAS: no se encontro la hoja '{hoja}'; se usa la unica "
                f"hoja disponible '{xl.sheet_names[0]}' en su lugar.")
...
def load_crac() -> pd.DataFrame:
    ...
    if hoja not in xl.sheet_names:
        if len(xl.sheet_names) == 1:
            log(f"CRAC: no se encontro la hoja '{hoja}'; se usa la unica "
                f"hoja disponible '{xl.sheet_names[0]}' en su lugar.")
            hoja = xl.sheet_names[0]
        else:
            raise ValueError(
